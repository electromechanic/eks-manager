...
class Repo(object):
    def __init__(self, cluster_name=None, environment=None, region=None, vpc=None, dry_run=False, debug=False):
        ...
        logger.debug(f"Repo object created with cluster_name={cluster_name}, environment={environment}, region={region}, vpc={vpc}, dry_run={dry_run}, debug={debug}")
    ...
    def _get_eks_versions(self):
        ...
        if returncode != 0:
            logger.error(f"Failed to get EKS versions. Return code: {returncode}, stderr: {stderr}")
            return []
...
@click.group()
@click.option("--cluster", "-c", envvar="EKS_CLUSTER", default='test', help="EKS Cluster name")
